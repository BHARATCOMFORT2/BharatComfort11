 ...
 * - POST /api/partners/listings/create       -> create listing
 * - POST /api/partners/listings/update       -> update listing
 * - POST /api/partners/listings/delete       -> delete listing
 * - GET  /api/partners/listings/list?page=.. -> list
 */
...
export default function PartnerListingsManager() {
  ...
  async function loadListings(pageNum = 1, tk = token) {
    try {
      setLoadBusy(true);
      const res = await fetch(`/api/partners/listings/list?page=${pageNum}&limit=20`, {
  ...
  async function handleFilesSelected(filesList: FileList | null) {
    ...
    if (total > MAX_IMAGES) {
      alert(`You can upload up to ${MAX_IMAGES} images total. Remove some first.`);
      return;
    }
  ...
  async function handleCreateOrUpdate() {
    ...
    if (images.length > MAX_IMAGES) return alert(`Max ${MAX_IMAGES} images`);
  ...
  function startEdit(l: Listing) {
    ...
    const exImages: ImageItem[] = (l.images || []).slice(0, MAX_IMAGES).map((url, idx) => ({
      id: `${Date.now()}_${idx}`,
      url,
      isExisting: true,
      ...
      >
        ...
        <div className="absolute left-1 top-1 flex gap-1">
          <button
            ...
            className={`px-2 py-1 rounded text-xs ${it.isPrimary ? "bg-yellow-400 text-black" : "bg-white/80 text-black"}`}
          >
            ★
          </button>
        </div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white p-6 rounded-xl shadow space-y-4 border">
        <h2 className="text-xl font-semibold">{editId ? "Edit Listing" : "Add New Listing"}</h2>
        ...
        >
          ...
          <p className="text-xs text-gray-500 mt-2">Max {MAX_IMAGES} images. New images will be compressed automatically.</p>
        </div>
        ...
        {images.length > 0 && (
          <div className="mt-3 grid grid-cols-3 sm:grid-cols-5 gap-3">
            {images.map((it, i) => (
              <Thumbnail key={i + (it.isExisting ? "_e" : "_n")} it={it} idx={i} />
            ))}
          </div>
        )}

        {/* Primary hint */}
        <p className="text-xs text-gray-500 mt-1">Click ★ to mark primary (primary image will be shown first).</p>

        <label className="flex items-center gap-2 mt-2 cursor-pointer">
          <input
            type="checkbox"
            checked={form.allowPayAtHotel}
            onChange={(e) => setForm({ ...form, allowPayAtHotel: e.target.checked })}
        ...
        <div className="flex gap-2">
          <Button onClick={handleCreateOrUpdate} className="bg-blue-600 text-white px-6 py-2 rounded" disabled={busy}>
      ...
      <div className="bg-white p-6 rounded-xl shadow border">
        ...
        {loadBusy ? <p>Loading...</p> : listings.length === 0 ? (
          <p className="text-gray-500">No listings found.</p>
        ) : (
          <div className="space-y-4">
            {listings.map((l) => (
              <div key={l.id} className="flex justify-between items-center border-b pb-3">
                <div>
                  <h3 className="font-semibold">{l.title}</h3>
                  <p className="text-sm text-gray-600">{l.location}</p>
                  <p className="text-sm text-gray-600">₹{l.price}</p>
                  {l.allowPayAtHotel && <p className="text-xs text-green-600 mt-1">✅ Pay at Hotel Enabled</p>}
                </div>

                <div className="flex gap-2">
                  <Button onClick={() => startEdit(l)} className="bg-yellow-500 hover:bg-yellow-600">Edit</Button>
                  <Button onClick={() => handleDelete(l.id)} className="bg-red-600 hover:bg-red-700">Delete</Button>
                </div>
              </div>
            ))}

            <div className="text-center mt-4">
              <Button onClick={() => loadListings(page + 1)} className="bg-gray-900 text-white" disabled={loadBusy}>
