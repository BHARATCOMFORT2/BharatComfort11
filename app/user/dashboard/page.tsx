...
export default function UserDashboard() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const totalSpent = bookingsSnap.docs.reduce((sum, b) => sum + ((b.data().amount as number) || 0), 0);
      const upcoming = bookingsSnap.docs.filter(b => new Date((b.data().date as string) || "").getTime() > new Date().getTime()).length;
