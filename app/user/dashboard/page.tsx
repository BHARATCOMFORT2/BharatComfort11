    ...
    const unsubAuth = auth.onAuthStateChanged(async (user) => {
      ...
      try {
        ...
        const unsubBookings = onSnapshot(bookingsQuery, (snap) => {
          ...
          setRecommended(
            allStays.filter((s) => !bookedIds.includes(s.id)).slice(0, 6)
          );
        });
        ...
        const unsubStays = onSnapshot(staysQuery, (snap) => {
          ...
          // Update recommended if bookings already loaded
          setRecommended((prev) => {
            const bookedIds = recentBookings.map((b) => b.listingId).filter(Boolean);
            return stays.filter((s) => !bookedIds.includes(s.id)).slice(0, 6);
          });
        });
...
// Section component with booking button
function Section({
  title,
  items,
  onBook,
